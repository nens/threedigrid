...
@click.option(
    "--grid-file",
    prompt="Path to the admin file",
    help="Path to the admin file",
    type=click.Path(exists=True),
)
@click.option(
    "--ipy/--no-ipy", default=False, help="Start an interactive ipython session"
)
...
@click.option("--model", type=click.Choice(["nodes", "lines", "breaches", "levees"]))
@click.option(
    "--subset",
    default="",
    help="Filter by a subset like 1D_all " "(applies only for models nodes and lines",
)
